...
lemma bounded_of_const {f : ℕ → ℕ} (h : f ∈ GrowthRate.const) : ∃ C, ∀ n, f n ≤ C := by
  obtain ⟨C, hC⟩ := h.exists_nonneg
  simp_all [Asymptotics.IsBigOWith]
  exact ⟨⌊C⌋₊ + ∑ n ∈ Finset.range hC.2.choose, f n, fun n ↦ if hn : n < hC.2.choose then le_add_of_nonneg_of_le (Nat.zero_le _) (Finset.single_le_sum (fun x _ ↦ Nat.zero_le (f x)) (Finset.mem_range.mpr hn)) else le_add_of_le_of_nonneg (Nat.le_floor <| hC.2.choose_spec n (le_of_not_gt hn)) (Nat.zero_le _)⟩

theorem bigO_add {f g a b : ℕ → ℕ} (hf : f ∈ bigO a) (hg : g ∈ bigO b) :
    f + g ∈ bigO (a + b) := by
  rw [bigO] at *
  simp_all [Asymptotics.IsBigO]
  simp_all [Asymptotics.IsBigOWith]
  norm_num [Norm.norm]
  obtain ⟨c₁, a₁, h₁⟩ := hf; obtain ⟨c₂, a₂, h₂⟩ := hg; use Max.max c₁ c₂; use Max.max a₁ a₂; intros n hn; rw [abs_of_nonneg, abs_of_nonneg] <;> nlinarith [h₁ n (le_trans (le_max_left _ _) hn), h₂ n (le_trans (le_max_right _ _) hn), le_max_left c₁ c₂, le_max_right c₁ c₂, le_max_left a₁ a₂, le_max_right a₁ a₂, Nat.zero_le (a n), Nat.zero_le (b n)]
...
theorem linear_comp (hf : f ∈ linear) (hg : g ∈ S) : f ∘ g ∈ S := by
  obtain ⟨C, hC⟩ : ∃ C : ℕ, ∀ n, f n ≤ C * (n + 1) := by
    obtain ⟨C, N, hC⟩ : ∃ C N, ∀ n ≥ N, f n ≤ C * n := by
      obtain ⟨C, hC⟩ := Asymptotics.isBigO_iff.mp hf
      obtain ⟨N, hN⟩ := Filter.eventually_atTop.mp hC; use ⌈C⌉₊, N; intros n hn; specialize hN n hn; norm_num [Norm.norm] at hN; exact_mod_cast hN.trans (mul_le_mul_of_nonneg_right (Nat.le_ceil _) <| Nat.cast_nonneg _)
    exact ⟨Max.max C (Finset.sup (Finset.range N) f), fun n ↦ if hn : n < N then Finset.le_sup (f := f) (Finset.mem_range.mpr hn) |> le_trans <| by nlinarith [le_max_right C (Finset.sup (Finset.range N) f)] else le_trans (hC n <| le_of_not_gt hn) <| by nlinarith [le_max_left C (Finset.sup (Finset.range N) f)]⟩
  have h_linear : ∀ C : ℕ, C * (g + 1) ∈ S := by
    intro C; induction C <;> simp_all [add_mul]
    · exact @GrowthRate.zero_mem S ‹_›
    · exact (add ‹_› (add hg one_mem))
  exact mem_dominating (Filter.Eventually.of_forall fun n ↦ by simpa using hC _) (h_linear C)
...
instance : LawfulGrowthRate log := by
  ...
  intro k hk g hg
  obtain ⟨C, hC⟩ := hk.exists_nonneg
  simp_all [Asymptotics.IsBigOWith]
  apply Asymptotics.isBigO_iff.mpr
  obtain ⟨M, hM⟩ : ∃ M, ∀ b < hC.right.choose, k b ≤ M := by
    exact ⟨Finset.sup (Finset.range hC.2.choose) k, fun b hb ↦ Finset.le_sup (f := k) (Finset.mem_range.mpr hb)⟩
  refine' ⟨Max.max C M + 1, Filter.eventually_atTop.mpr ⟨hC.2.choose + 1, fun x hx ↦ _⟩⟩
  norm_cast
  simp_all
  by_cases hgx : g x < hC.right.choose
  · refine' le_trans _ (le_mul_of_one_le_right _ _)
    · exact le_add_of_le_of_nonneg (le_trans (Nat.cast_le.mpr (hM _ hgx)) (le_max_right _ _)) zero_le_one
    · positivity
    · exact_mod_cast Nat.le_log_of_pow_le (by norm_num) (by linarith)
  · apply le_trans (hC.2.choose_spec _ (le_of_not_gt hgx))
    exact mul_le_mul (by linarith [le_max_left C M, le_max_right C M]) (mod_cast Nat.log_mono_right (hg x)) (by positivity) (by positivity)
...
lemma polylog_mem_dominating {f g : ℕ → ℕ} (h : ∀ᶠ x in atTop, g x ≤ f x)
  ...
  obtain ⟨C, hf⟩ := hf
  exact ⟨C, (Asymptotics.IsBigO.trans (Asymptotics.isBigO_iff.mpr ⟨1, by
    filter_upwards [h] with n hn; simpa using mod_cast hn⟩) hf)⟩
...
instance : LawfulGrowthRate linearithmic := by
  ...
  · intro k hk g hg
    ...
    apply Asymptotics.IsBigO.of_bound ((∑ n ∈ Finset.range N, k n) + C) _
    simp +zetaDelta at *
...
lemma isBigO_comp_bound_plus_const {f h : ℕ → ℕ} (hf : (f · : ℕ → ℤ) =O[.atTop] (h · : ℕ → ℤ)) (g : ℕ → ℕ) :
  ...
  apply Asymptotics.IsBigO.of_bound (Max.max M (|C|))
...
instance : LawfulGrowthRate quasilinear where
  ...
  comp_le_id := by
    ...
    have h_comp_bound_plus_one : (fun n ↦ ((g n) * (Nat.log 2 (g n)) ^ C : ℤ) + 1) =O[.atTop] (fun n ↦ (n * (Nat.log 2 n) ^ C : ℤ)) := by
      exact h_comp_bound.add (one_isBigO_quasilinear_bound C)
...
instance : LawfulGrowthRate e_pow := by
  ...
  · intro k hk g hg
    ...
    simpa using mod_cast h_comp n |> le_trans <| mul_le_mul_of_nonneg_left (Nat.ceil_mono <| Real.exp_le_exp.mpr <| Nat.cast_le.mpr <| hg n) <| Nat.cast_nonneg _
...
/-
For f ∈ polylog, there exists k with f = O((log n)^k). We need f ∈ sqrt = bigO(Nat.sqrt). Since (log n)^k / √n → 0 as n → ∞ (any power of log grows slower than √n), eventually (log n)^k ≤ √n. From f(n) ≤ c * (log n)^k and (log n)^k ≤ √n eventually, we get f(n) ≤ c * √n eventually. For the limit argument, use Real.tendsto_pow_mul_exp_neg_atTop_nhds_zero and the substitution y = log x, so (log x)^k / √x = (log x)^k / x^(1/2) → 0. Then convert this to the Nat.sqrt version. Use polylog_is_littleO_sqrt which is already proved in this file, or prove directly.
-/
theorem polylog_subset_sqrt : polylog ⊆ sqrt := by
  intro f hf
  obtain ⟨C, hC⟩ := hf
  have h_bound : ∃ N, ∀ n ≥ N, (f n : ℝ) ≤ (Nat.sqrt n : ℝ) := by
    have h_bound : Filter.Tendsto (fun n : ℕ ↦ (Nat.log 2 n : ℝ) ^ C / Real.sqrt n) Filter.atTop (nhds 0) := by
      -- We can convert this statement into a form that allows us to apply the squeeze theorem.
      suffices h_convert : Filter.Tendsto (fun n : ℕ ↦ (Real.log n / Real.log 2) ^ C / Real.sqrt n) Filter.atTop (nhds 0) by
        refine squeeze_zero_norm' ?_ h_convert
        norm_num [abs_div, abs_of_nonneg, Real.sqrt_nonneg]
        refine ⟨2, fun n hn ↦ ?_⟩
        gcongr
        rw [le_div_iff₀ (Real.log_pos one_lt_two), ← Real.log_pow]; exact Real.log_le_log (by positivity) (mod_cast Nat.pow_log_le_self _ <| by positivity)
      -- We can convert this statement into a form that allows us to apply the squeeze theorem by substituting $y = \log n$.
      suffices h_convert : Filter.Tendsto (fun y : ℝ ↦ (y / Real.log 2) ^ C / Real.exp (y / 2)) Filter.atTop (nhds 0) by
      ...
      -- We can simplify the expression inside the limit further by substituting $z = y / 2$.
      suffices h_simplify'' : Filter.Tendsto (fun z : ℝ ↦ (2 * z / Real.log 2) ^ C / Real.exp z) Filter.atTop (nhds 0) by
        convert h_simplify''.comp (Filter.tendsto_id.atTop_mul_const (by norm_num : 0 < (2⁻¹ : ℝ))) using 2; norm_num; ring_nf
      suffices h_factor : Filter.Tendsto (fun z : ℝ ↦ z ^ C / Real.exp z) Filter.atTop (nhds 0) by
        convert h_factor.const_mul ((2 / Real.log 2) ^ C) using 2 <;> ring
      simpa [Real.exp_neg] using Real.tendsto_pow_mul_exp_neg_atTop_nhds_zero C
    obtain ⟨k, hk⟩ : ∃ k, ∀ᶠ n in Filter.atTop, (f n : ℝ) ≤ k * (Nat.log 2 n : ℝ) ^ C := by
      rw [Asymptotics.isBigO_iff'] at hC; aesop
    have h_combined : ∃ N, ∀ n ≥ N, (k * (Nat.log 2 n : ℝ) ^ C) / Real.sqrt n ≤ 1 := by
      simpa [mul_div_assoc] using h_bound.const_mul k |> fun h ↦ h.eventually (ge_mem_nhds <| by norm_num)
    obtain ⟨N, hN⟩ := h_combined; obtain ⟨M, hM⟩ := Filter.eventually_atTop.mp hk; use Max.max N M + 1; intros n hn; specialize hN n (by linarith [le_max_left N M, le_max_right N M]); specialize hM n (by linarith [le_max_left N M, le_max_right N M]); rw [div_le_iff₀] at hN <;> norm_num at *
    · exact Nat.le_sqrt.mpr (by rw [← @Nat.cast_le ℝ]; push_cast; nlinarith [Real.mul_self_sqrt (Nat.cast_nonneg n)])
    · grind
  obtain ⟨N, hN⟩ := h_bound
  have h_eventually : ∀ n ≥ N, f n ≤ Nat.sqrt n := by
    exact_mod_cast hN
  have h_bigO : (f · : ℕ → ℤ) =O[.atTop] (Nat.sqrt · : ℕ → ℤ) := by
    exact Asymptotics.isBigO_iff.mpr ⟨1, Filter.eventually_atTop.mpr ⟨N, fun n hn ↦ by simpa using mod_cast h_eventually n hn⟩⟩
  exact h_bigO
...
private lemma linear_mul_log_comp_linear {g : ℕ → ℕ} (hg : g ∈ GrowthRate.linear) :
  ...
  apply GrowthRate.linearithmic_of_linear_mul_log hg (log_comp_linear hg)
...
theorem quasipoly_comp (hf : f ∈ quasipoly) (hg : g ∈ quasipoly) : f ∘ g ∈ quasipoly := by
  ...
  have h_subst : ∃ C'' : ℕ, ∀ᶠ n in .atTop, (Nat.log 2 (f (g n)) : ℝ) ≤ C'' * (Nat.log 2 n : ℝ) ^ (Dg * Df) := by
    ...
    · apply le_trans (Nat.cast_le.mpr ‹_›)
...
private lemma computable_max_scan {f : ℕ → ℕ} (hf : Computable f) : Computable (max_scan f) := by
  apply Computable.id.nat_rec (h := fun _ p ↦ Nat.max p.2 (f (p.1 + 1))) (Computable.const _)
  -- The function p ↦ p.2.max (f (p.1 + 1)) is computable because it is a composition of computable functions.
  apply Computable₂.comp₂ (β := ℕ × ℕ) (f := fun (p : ℕ × ℕ) (q : ℕ × ℕ) ↦ Nat.max p.2 q.2) (g := fun _ p ↦ (p.1, p.2)) (h := fun _ p ↦ (p.1, f (p.1 + 1)))
  · have h_max : Computable₂ (fun (p q : ℕ) ↦ Nat.max p q) := by
      exact Primrec₂.to_comp Primrec.nat_max
    exact h_max.comp (Computable.snd.comp .fst) (Computable.snd.comp .snd)
